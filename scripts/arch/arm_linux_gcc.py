...
def arch_env_init(root_dir):
    ...
    # 编译器地址
    COMPILER_PATH = path_helper.get_abs_path("/usr/local/arm_linux_4.8/bin")
    # C 编译器路径
    CC = path_helper.path_join(COMPILER_PATH, "arm-linux-gcc")
    # C++ 编译器路径
    CXX = path_helper.path_join(COMPILER_PATH, "arm-linux-g++")
    # 汇编器路径
    AS = path_helper.path_join(COMPILER_PATH, "arm-linux-as")
    # 静态库归档工具
    AR = path_helper.path_join(COMPILER_PATH, "arm-linux-ar")
    # 链接器路径
    LD = path_helper.path_join(COMPILER_PATH, "arm-linux-ld")
    # 通用编译选项
